...
const Icon = ({ name, size = 24, className = "" }) => {
  // Common wrapper style to ensure identical dimensions and centering
  const wrapperStyle = {
    width: size,
    height: size,
    display: "flex",
  ...
  if (CUSTOM_ICONS[name]) {
    ...
    return (
      <div style={wrapperStyle} className={className}>
        <CustomIcon size={size} className="w-full h-full object-cover" />
      </div>
    );
  }
  ...
  return (
    <div style={wrapperStyle} className={className}>
      <FinalIcon size={size} strokeWidth={2} />
    </div>
  );
};
